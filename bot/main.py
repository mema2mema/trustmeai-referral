# TrustMe AI Telegram Bot — v3.9.7 (webhook-stable)
...
async def subscribe(update, context): add_subscriber(update.effective_chat.id); await update.message.reply_html('✅ Subscribed to alerts.')
...
async def graph_cmd(update, context):
    ...
    try:
        ...
        if 'equity' not in df.columns:
            start_equity=float(os.getenv('START_EQUITY','1000')); df['equity']=start_equity+df['pnl'].cumsum()
...
async def balance_cmd(update, context):
    uid=update.effective_chat.id; from .wallet import Wallet
    bal=Wallet(WALLET_FILE).balance(uid); await update.message.reply_html(f'💼 <b>Mock USDT Balance:</b> <code>{bal:.2f}</code>')
...
async def withdraw_cmd(update, context):
    ...
    try:
        ...
        if admin_id:
            kb=InlineKeyboardMarkup([[InlineKeyboardButton('✅ Approve', callback_data=f'wd:approve:{wd["id"]}'), InlineKeyboardButton('❌ Deny', callback_data=f'wd:deny:{wd["id"]}')]])
            try:
                await context.bot.send_message(chat_id=admin_id, text=f'💸 <b>Withdraw Request</b>\nID: <code>{wd["id"]}</code>\nUser: <code>{uid}</code>\nAmount: <b>{amount:.2f}</b> USDT', parse_mode=ParseMode.HTML, reply_markup=kb)
            except Exception as e: log.warning(f'Admin notify failed: {e}')
...
async def handle_document(update, context):
    ...
    except Exception as e: await update.message.reply_text(f'Analyze failed: {e}')
